...
def test_standing_freezes_pose_and_never_resets_at_clip_boundary(monkeypatch, frames):
    ...
    path = Path(__file__).resolve().parents[1] / (
        'source/whole_body_tracking/whole_body_tracking/tasks/tracking/config/t1_atlas/motion.py')
    ...
    command = module.AtlasStandingMotion(None, SimpleNamespace(motion=motion))
    for _ in range(20):
