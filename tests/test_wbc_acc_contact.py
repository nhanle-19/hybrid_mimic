...
from wbc_acc_contact import GroundContactEstimator, read_ground_forces
...
def action(monkeypatch):
    ...
    controller = ModuleType('wbc_acc_contact_test.controller_cfg')
    controller.T1WBCACCControllerCfg = type('T1WBCACCControllerCfg', (), {})
    for name, module in [('isaaclab.managers', managers), ('isaaclab.utils', utils),
                         ('wbc_acc_contact_test.controller_cfg', controller)]:
        monkeypatch.setitem(sys.modules, name, module)
    path = ROOT/'source/whole_body_tracking/whole_body_tracking/tasks/tracking/config/t1_wbc_acc/wbc_acc_action.py'
    spec = importlib.util.spec_from_file_location('wbc_acc_contact_test.wbc_acc_action', path)
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    term = object.__new__(module.WBCACCAction)
