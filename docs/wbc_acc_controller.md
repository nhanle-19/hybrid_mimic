# WBC-ACC reference-conditioned contact policy
...
WBC-ACC checkpoints are incompatible; train a new policy.
...
`scripts/export_wbc_acc_contacts.py` exports its bottom-face vertices and source
hashes to `wbc_acc_contacts.json`. The existing four-corner rho representation now
...
the normal-force-weighted squared excess contact-point speed above 0.05 m/s,
gated by measured ground normal force above 1 N. There is no feasibility-only reward.
...
WBC-ACC rewards retain body pose/velocity imitation, joint-position limits, and
...
toe pivot is therefore unpenalized. An alive reward adds +1 per simulated
second survived, excluding failure steps and including timeouts.
...
```bash
python scripts/check_wbc_acc_contact_policy.py
python -m pytest tests/test_wbc_acc_gpu.py tests/test_wbc_acc_qp.py tests/test_wbc_acc_contact.py tests/test_wbc_acc_standing.py tests/test_exporter_metadata.py -q
```

These use prescribed kinematic snapshots, not realized simulator trajectories.
The validation report is `eval_data/wbc_acc/reference_contact_validation/`.

On a host with a working GPU, test manual standing before training:

```bash
python scripts/rsl_rl/eval_wbc.py --task Standing-WBC-ACC-T1-v0 \
...
`Tracking-WBC-ACC-T1-Eval-v0`; the QP still follows actual geometry. The offline
slow-lift scenario above is not a substitute for this closed-loop test.

Short training smoke command:

```bash
python scripts/rsl_rl/train.py --task Tracking-WBC-ACC-T1-v0 \
  --motion_file retargeted_motion/g18_push_kick_right_t1_training.npz \
