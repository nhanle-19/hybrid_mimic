...
CONTACT_PATH = Path(__file__).resolve().parents[1]/'assets/booster/t1/wbc_acc_contacts.json'
...
def validate_contact_asset():
    ...
    for name, expected in data['source_layers_sha256'].items():
        ...
        if len(matches) != 1 or hashlib.sha256(matches[0].read_bytes()).hexdigest() != expected:
            raise ValueError(f'WBCACC collider export is stale for {name}; regenerate wbc_acc_contacts.json')
