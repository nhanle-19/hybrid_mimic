"""WBCACC-style centroidal QP with physical point contacts and hard torque bounds.
...
class WBCACCQP:
    ...
    def solve(self, state, desired_rate, contacts, tasks=(), external=(), contact_weights=None):
        # None retains the fully hard reference solver for numerical comparisons.
        # Explicit weights soften only the two translations tangent to each normal.
        if contact_weights is not None:
            if set(contact_weights) != {c.body for c in contacts} or any(
                    not np.isfinite(w) or w <= 0 for w in contact_weights.values()):
                raise ValueError('Provide a positive finite contact weight for each active foot')
        nv = state['M'].shape[0]
        ...
        hard_stance = []
        for contact, frame in stance:
            if contact_weights is None:
                hard_j, hard_bias = frame['J'], frame['bias']
            else:
                normal = np.asarray(contact.normal, dtype=float)
                normal /= np.linalg.norm(normal)
                # I-nn^T projects translation onto the ground tangent plane.
                tangent = np.eye(3)-np.outer(normal, normal)
                j = np.hstack([tangent@frame['J'][:3], np.zeros((3, nr))])
                bias = tangent@frame['bias'][:3]
                weight = contact_weights[contact.body]
                hessian += weight*j.T@j
                linear += weight*j.T@bias
                hard_j = np.vstack([normal@frame['J'][:3], frame['J'][3:]])
                hard_bias = np.r_[normal@frame['bias'][:3], frame['bias'][3:]]
            eq.append(np.hstack([hard_j, np.zeros((len(hard_bias), nr))]))
            rhs.append(-hard_bias)
            hard_stance.append((hard_j, hard_bias))
        for task in tasks:
        ...
        if result.info.status_val != 1 or result.x is None or not np.isfinite(result.x).all():
            raise RuntimeError(f'WBCACC QP failed: {result.info.status}; no clipped/fallback torque applied')
        x = result.x
        ...
        stance_residual = np.concatenate([j@acceleration+b for j, b in hard_stance]) if stance else np.zeros(0)
        ...
        if max(metrics.values()) > self.tolerance:
            raise RuntimeError(f'WBCACC QP physical residual check failed: {metrics}')
        return dict(acceleration=acceleration, rho=rho, torque=torque, rate=rate,
                    ...
                    contact_acceleration={c.body: f['J']@acceleration+f['bias'] for c, f in stance},
                    contact_weights=contact_weights, iterations=result.info.iter, variable_count=nx)
