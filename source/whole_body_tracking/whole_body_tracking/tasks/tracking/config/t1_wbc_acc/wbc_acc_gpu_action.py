"""Policy-rate WBCACC QP with zero-order-held torques between updates."""
import numpy as np
import torch

from isaaclab.managers import ActionTerm
from .wbc_acc_action import WBCACCAction
from whole_body_tracking.utils.wbc_acc_torch_model import WBCACCTorchModel, rotation_xyzw, mv
from whole_body_tracking.utils.wbc_acc_gpu_control import WBCACCGPUQP, FEET, select_state
from whole_body_tracking.utils.wbc_acc_contact_policy import available_vertices, finite_difference, validate_contact_asset


class WBCACCGPUAction(WBCACCAction):
    solver_type = WBCACCGPUQP
    action_name = 'wbc_acc'
    log_prefix = 'WBCACC'

    def __init__(self, cfg, env):
        ActionTerm.__init__(self, cfg, env)
        if cfg.backend not in ('batched', 'osqp'):
            raise ValueError('WBC backend must be batched or osqp')
        if cfg.backend == 'batched' and torch.device(env.device).type != 'cuda':
            raise ValueError(f'{self.log_prefix} GPU training requires --device cuda:0 (no CPU fallback). '
                             f'Use actions.{self.action_name}.backend=osqp explicitly for CPU evaluation.')
        if cfg.contact_source not in ('reference', 'ground_force'):
            raise ValueError('WBCACC contact_source must be reference or ground_force')
        ...
        self.dynamics, self.feet = WBCACCTorchModel(env.device), FEET
        if set(self._asset.joint_names) != set(self.dynamics.joint_names):
            raise ValueError('WBCACC model and simulation joint names differ')
        self.pin_to_sim = [self._asset.joint_names.index(n) for n in self.dynamics.joint_names]
        self._raw = torch.zeros((env.num_envs, self.action_dim), device=env.device)
        ...
        if bool((self._asset.data.joint_stiffness != 0).any() | (self._asset.data.joint_damping != 0).any()):
            raise ValueError('WBCACC total torque bounds require zero actuator PD gains')
        self.solver = self.solver_type(cfg.controller, cfg.friction, cfg.residual_tolerance,
        ...
        print(f'[INFO] {self.log_prefix} backend={cfg.backend}: {env.num_envs} environments; dynamics and QP on {env.device}, float64')
    ...
    def apply_actions(self):
        ...
        # Actuator PD, friction and armature are zero in configure_wbc_acc: these
        ...
        logs[f'{self.log_prefix}/solver_failure_fraction'] = failed.float().mean()
        logs[f'{self.log_prefix}/solver_failures_total'] = self.failure_count.sum().float()
        logs[f'{self.log_prefix}/joint_tracking_rmse'] = tracking_error.mean()
        logs[f'{self.log_prefix}/torque_saturation_fraction'] = saturation.mean()
        logs[f'{self.log_prefix}/predicted_normal_force'] = result['wrenches'][..., 2].mean()
        logs[f'{self.log_prefix}/measured_normal_force'] = ground[..., 2].mean()
        if self.previous_wrench is not None and self._clock % self._env.cfg.decimation == 0:
            ...
            for foot in range(2):
                for axis, label in enumerate(('fx', 'fy', 'fz', 'mx', 'my', 'mz')):
                    logs[f'{self.log_prefix}/{FEET[foot]}_predicted_{label}'] = self.previous_wrench[:, foot, axis].mean()
                    logs[f'{self.log_prefix}/{FEET[foot]}_actual_{label}'] = measured[:, foot, axis].mean()
            logs[f'{self.log_prefix}/contact_wrench_prediction_rmse'] = (measured-self.previous_wrench).square().mean().sqrt()
        ...
        if self.cfg.record_diagnostics:
            self.last_prediction = dict(_valid=torch.ones_like(failed), env_id=torch.arange(self._env.num_envs, device=self._env.device),
                ...
                normal_force_limits=result['normal_force_limits'], available_vertices=masks,
            ...
            if 'contact_weights' in result:
                self.last_prediction['contact_weights'] = result['contact_weights']
        self._asset.set_joint_effort_target(self._torques)
    ...
    def save_diagnostics(self, path, finalize=True, completed=True, failure_reason=''):
        ...
        if finalize:
            self._finish_pending(None)
        if not self.records:
            raise RuntimeError('No WBCACC diagnostics recorded')
        ...
        np.savez_compressed(path, **{k: np.asarray([r[k] for r in self.records]) for k in self.records[0]},
            ...
            policy_type=np.asarray(self.action_name), policy_action_dim=self.action_dim,
