"""WBCACC action interface; active backends use a policy-rate direct-torque QP."""
import numpy as np
import torch

from isaaclab.managers import ActionTerm, ActionTermCfg
from isaaclab.utils import configclass
from .controller_cfg import T1WBCACCControllerCfg


class WBCACCAction(ActionTerm):
    def __new__(cls, cfg, env):
        if cls is WBCACCAction:
            if cfg.backend in ('batched', 'osqp'):
                from .wbc_acc_gpu_action import WBCACCGPUAction
                return object.__new__(WBCACCGPUAction)
            if cfg.backend != 'osqp':
                raise ValueError('WBCACC backend must be batched or osqp')
        return object.__new__(cls)

    def __init__(self, cfg, env):
        super().__init__(cfg, env)
        # Optional dependencies are loaded only when this task is instantiated.
        from whole_body_tracking.utils.wbc_acc_model import WBCACCModel, FEET
        from whole_body_tracking.utils.wbc_acc_qp import WBCACCQP
        from whole_body_tracking.utils.wbc_acc_contact import GroundContactEstimator
        ...
        self.solvers = [WBCACCQP(lim, momentum_weights=cfg.controller.momentum_weights,
    ...
    def process_actions(self, actions):
        if actions.shape != self._raw.shape or not torch.isfinite(actions).all():
            raise ValueError(f'WBC expects {self.action_dim} finite policy actions per environment')
        self._raw[:] = actions
    ...
    def _reference_states(self):
        from whole_body_tracking.utils.wbc_acc_control import reference_contact_schedule
    ...
    def _ground_forces(self):
        from whole_body_tracking.utils.wbc_acc_contact import read_ground_forces
    ...
    def _validate_simulator_model(self, states):
        ...
        for name, body in self.dynamics.description['bodies'].items():
            ...
            if not np.allclose(masses[:, index], body['mass'], atol=1e-5):
                raise RuntimeError(f'WBCACC simulator mass mismatch for {name}')
            ...
            if not np.allclose(actual, expected, atol=2e-4):
                raise RuntimeError(f'WBCACC simulator frame mismatch for {name}: max error {np.abs(actual-expected).max()}')
            com = local_com_poses[:, index, :3]
            if not np.allclose(com, body['com'], atol=1e-5):
                raise RuntimeError(f'WBCACC simulator CoM mismatch for {name}')
            ...
            if not np.allclose(actual_inertia, expected_inertia, atol=1e-5):
                raise RuntimeError(f'WBCACC simulator inertia mismatch for {name}')
        for state in states:
            if not np.allclose(state['measured_momentum'], state['momentum'], atol=1e-3):
                raise RuntimeError('WBCACC simulator/analytical centroidal momentum mismatch')
        self.validated = True
    ...
    def apply_actions(self):
        from whole_body_tracking.utils.wbc_acc_qp import Contact
        from whole_body_tracking.utils.wbc_acc_control import build_reference_tasks, swing_tasks, force_diagnostics, contact_weights
    ...
    def save_diagnostics(self, path, finalize=True, completed=True, failure_reason=''):
        ...
        if not self.records:
            raise RuntimeError('No WBCACC diagnostic samples recorded')
...
@configclass
class WBCACCActionCfg(ActionTermCfg):
    class_type: type = WBCACCAction
    asset_name: str = 'robot'
    backend: str = 'batched'
    batched_max_iterations: int = 100
    batched_tolerance: float = 1e-9
    controller: T1WBCACCControllerCfg = T1WBCACCControllerCfg()
    friction: float = .6
