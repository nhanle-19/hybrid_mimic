"""Plot WBC-ACC/WBC-FORCE diagnostics with separate predictions and measurements."""
import argparse
...
def main():
    ...
    p.add_argument('--input', type=Path, default=Path('eval_data/wbc_acc/diagnostics.npz'))
    p.add_argument('--output_dir', type=Path, help='Defaults to a plots folder beside the input NPZ.')
    args = p.parse_args()
    args.output_dir = args.output_dir or args.input.parent/'plots'
        ...
        if 'predicted_contact_wrench' in data:
            ...
            for foot in range(2):
                for component, label in enumerate(('Fx (N)', 'Fy (N)', 'Fz (N)', 'Mx (Nm)', 'My (Nm)', 'Mz (Nm)')):
                    ...
                    if component == 2 and 'normal_force_limits' in data:
                        ax.plot(t, data['normal_force_limits'][mask, foot], '--', label='QP upper limit')
            ...
            if 'contact_weights' in data:
                ...
                axes[1].plot(t, data['normal_force_limits'][mask]); axes[1].set_ylabel('Normal-force limits (N)')
    ...
    print(f'Saved WBC plots to {args.output_dir}')
