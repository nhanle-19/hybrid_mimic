...
from wbc_acc_control import force_diagnostics


def main():
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument('--output', type=Path, default=Path('eval_data/wbc_acc/numerical_residuals.json'))
    ...
    for feet in [(), FEET[:1], FEET[1:], FEET]:
        ...
        result = WBCACCQP(limits).solve(state, np.zeros(6), [Contact(f) for f in feet])
