...
from atlas_gpu_control import AtlasGPUQP
from atlas_contact_policy import FEET, sole_vertices, available_vertices, finite_difference


def main():
    torch.set_num_threads(4)
    path = ROOT/'source/whole_body_tracking/whole_body_tracking/tasks/tracking/config/t1_atlas/controller_cfg.py'
    ...
    for scenario in ('standing', 'slow_right_foot_lift'):
        ...
        result = AtlasGPUQP(cfg).solve(state, state, actions, geometry, limits)
    ...
    output = ROOT/'eval_data/atlas/reference_contact_validation/manual_checks.json'
